...
import { monsterName } from '../../config/entityNames';
...
export function showBestiaryDetail(scene: GameScene, x:number,y:number,w:number,h:number,nm:string,df:any,en:boolean,kl:number,pa:Phaser.GameObjects.Container):void {
    ...
    const ib=df.type==='妖将'||df.type==='妖王';const nc=ib?'#ffcc44':df.type==='恶妖'?'#ff8866':'#ddddff';dc.add(scene.add.text(pad,pad,monsterName(nm),{fontSize:'22px',color:nc,fontStyle:'bold',padding:{y:3}}));
